...
function testProcessNext(c, done) {
  ...
  function processNextNone(c, cb) {
    ...
    dbwrkr.processNext(c.ourQueue, function (err, nextItem) {
      expect(err).to.be(null);
...
function testMiddlewareOnce(c, done) {
  ...
  function verifyEvent(c, cb) {
    ...
    dbwrkr.find(findSpec, (err, events) => {
      ...
      const timeStart = new Date(Date.now() + onceMs - 200);
      const timeEnd = new Date(Date.now() + onceMs + 200);
...
function benchmark(c, done) {
  ...
  function publishEvents(c, cb) {
    ...
    return flw.times(numEvents/2, publish, cb);
    function publish(cb) {
  ...
  function receiveEvents(c, cb) {
    stopTime = new Date();
    console.log(`- published ${numEvents} events in ${stopTime - startTime} ms, receiving`);
    startTime = new Date();
    const checkTimeMs = 5;
    ...
    function nextCheck() {
      if (numEventsReceived === numEvents) {
        stopTime = new Date();
        console.log(`- received ${numEvents} events in ${stopTime - startTime} ms`);
        return cb();
      }
      setTimeout(nextCheck, checkTimeMs);
    }
