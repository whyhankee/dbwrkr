...
var debug = require('debug')('dbwrkr:tests');
...
function testPublish(c, done) {
  ...
  function verifyRemove(c, cb) {
    ...
    dbwrkr.find({eventName: c.event.event.name}, function (err, items) {
      expect(err).to.be(null);
...
function testProcess(c, done) {
  ...
  function processNextNone(c, cb) {
    ...
    dbwrkr.processNext(randomstring(), function (err, nextItem) {
      expect(err).to.be(null);
  ...
  function processNext(c, cb) {
    ...
    dbwrkr.processNext(c.event.queue, function (err, nextItem) {
      expect(nextItem.id).to.be(c.event.newEventIds[0]);
      expect(nextItem.name).to.be(c.event.event.name);
      expect(nextItem.queue).to.be(c.event.queue);
      expect(nextItem.tid).to.be(c.event.event.tid);
      expect(!nextItem.when);
      expect(nextItem.done).to.be.a(Date);
      expect(nextItem.retryCount).to.be(0);
      expect(nextItem.parent).to.be(null);
      expect(nextItem.payload).to.eql(c.event.event.payload);
      return cb();
    });
  }
}
...
function testEventFind(c, done) {
  ...
  function testFindSpec(findSpec, c, expected, cb) {
    var findKeys = Object.keys(findSpec);
    var isMultiple = Array.isArray(findSpec[findKeys[0]]);
...
function testEventFollowUp(c, done) {
  ...
  function fetchAndFollowUp(c, cb) {
    ...
    dbwrkr.fetchNext(c.event.queue, function(err, gotEvent) {
  ...
  function checkEvent(c, cb) {
    ...
    var newEventId = c.newEventIds[0];
...
function testEventRetry(c, done) {
  ...
  function verifyRetryEvent(c, cb) {
    ...
    dbwrkr.find({name: c.event.event.name}, function (err, events) {
      ...
      var retryEvents = events.filter(function(event) {
        return !!event.when;
      });
