...
/**
 * Get a representation of a Qeueu to receive it's events
 */
 ...
 * @param {string} event.tid targetId (e.g. userId)
 * @param {Date} event.when when the event must be processed (default: direct)
 * @param {Object} event.payload payload to send with the event
 * @param {function} done callback
...
DBWrkr.prototype.publish = function publish(events, done) {
  ...
  function runMiddleware(c, cb) {
    return flw.each(c.events, runEvent, (err) => {
      if (err) return cb(err);

      debug('publish (after middleware', events);
      return cb();
    });

    function runEvent(event, cb) {
  ...
  function filterEvents(c, cb) {
    c.events = c.events.filter(e => {
      if (!e.name) return false;
      if (e.__blocked) return false;
      return true;
    });
    debug('publish (after filter)', c.events);
    return cb();
  }

  function makeQueueEvents(c, cb) {
    return flw.each(c.events, makeEventsForQueues, cb);

    function makeEventsForQueues(event, cb) {
      debug('publish - makeQueueItems for event', event.name);
  ...
  function publishEvents(c, cb) {
    debug('publish - storing events', c.queueEvents);
    if (c.queueEvents.length === 0) return cb(null, []);

    return self.storage.publish(c.queueEvents, function (err, createdIds) {
      if (err) return cb(err);

      // Check for the return of createdIds
      assert(c.queueEvents.length === createdIds.length, 'created eventIds returned');
